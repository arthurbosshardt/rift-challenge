# Déploiement Rift Challenge (Render + Vercel + Supabase)
...
(`www.rift-challenge.com` redirige vers l'adresse sans `www`, voir `frontend/vercel.json`.)
...
Le repo contient un `render.yaml` (Blueprint) qui déclare déjà le service — Render → **New → Blueprint** le détecte automatiquement.

1. Connecter le dépôt GitHub à Render.
2. **New → Blueprint** (détecte `render.yaml`) ou **Web Service** manuel :
   - Root directory : `backend`
   - Runtime : **Docker**
   - Dockerfile : `backend/Dockerfile`
   - Health check : `/actuator/health`
3. Variables d'environnement (celles marquées `sync: false` dans `render.yaml` doivent être saisies manuellement dans le dashboard Render — elles ne sont pas dans le repo) :
   ...
   | `RIOT_REGIONAL_ROUTING` | `europe` (déjà fixé dans `render.yaml`) |
   | `CORS_ORIGINS` | `https://rift-challenge.com,https://www.rift-challenge.com` |
   | `HIKARI_MAX_POOL_SIZE` | `3` (déjà fixé dans `render.yaml` — le session pooler Supabase plafonne ~15 clients partagés) |
   | `HIKARI_MIN_IDLE` | `1` (déjà fixé dans `render.yaml`) |
...
4. **Brancher ton nom de domaine** (Vercel → ton projet → **Settings** → **Domains**) :
   ...
   **Étape A — Ajouter les deux adresses du site**
   ...
   - `rift-challenge.com` → l'adresse **sans** `www` (adresse principale)
   - `www.rift-challenge.com` → la variante **avec** `www`
   ...
   **Étape B — Configurer le DNS chez ton registrar**
   ...
   En résumé :
   - pour `rift-challenge.com` → Vercel indique quoi mettre (souvent type **A**, valeur une IP, ou **CNAME** `cname.vercel-dns.com`)
   ...
   **Étape C — Adresse principale et redirection www**
   ...
   - Le fichier `frontend/vercel.json` du repo redirige déjà `www.rift-challenge.com` vers `rift-challenge.com`.
...
5. Variables d'environnement (Vercel → Settings → Environment Variables) :
   ...
   Ces variables sont lues au build par `frontend/scripts/write-production-env.mjs`, qui génère `src/environments/environment.prod.ts` — ne pas committer ce fichier avec de vraies valeurs.
...
3. Auth Google : [GOOGLE_OAUTH.md](GOOGLE_OAUTH.md)
...
- La clé reste **uniquement** côté backend (variable Render, jamais exposée au front).
- La plateforme Riot (`euw1`, etc.) est choisie **par challenge** en base (`ChallengeRegion`), pas via une variable d'environnement globale.

## 6. Vérifications post-déploiement

- [ ] Health backend OK (`/actuator/health`)
